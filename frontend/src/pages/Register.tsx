    ...
    <div className="min-h-screen flex bg-nothing-black">
      ...
      <div className="hidden lg:flex lg:w-1/2 border-r border-nothing-grey-800 items-center justify-center p-16">
        <div className="max-w-sm">
          ...
          <p className="text-sm font-mono text-nothing-grey-500 leading-relaxed mb-12">
            Join developers using AI-powered intelligence to track growth, identify skill gaps, and accelerate their careers.
          </p>
          <div className="space-y-4">
            {['Connect GitHub in one click', 'Get your DevScore in minutes', 'Upload resume for AI analysis', 'Track growth over time'].map((text) => (
              <div key={text} className="flex items-center gap-3 text-nothing-grey-400">
                <div className="w-1 h-1 bg-nothing-red" />
                <span className="text-xs font-mono tracking-wide">{text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <h2 className="text-lg font-mono font-bold text-nothing-white mb-1">Create Account</h2>
          <p className="text-xs font-mono text-nothing-grey-500 mb-8 tracking-wide">Start your developer intelligence journey</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input label="Confirm" type="password" value={form.confirm} onChange={update('confirm')} placeholder="••••••••" required autoComplete="new-password" />
            <Button type="submit" variant="primary" className="w-full" isLoading={loading}>Create Account</Button>
          </form>
          ...
          <div className="mt-8">
            ...
            <a href={`${import.meta.env.VITE_API_URL || ''}/api/auth/github`} className="mt-4 flex items-center justify-center gap-3 w-full px-4 py-2.5 border border-nothing-grey-700 text-nothing-grey-300 text-xs font-mono uppercase tracking-wide hover:border-nothing-grey-500 hover:text-nothing-white transition-colors duration-300">
          ...
          <p className="mt-8 text-center text-xs font-mono text-nothing-grey-600">
            Already have an account?{' '}<Link to="/login" className="text-nothing-white hover:text-nothing-red transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
